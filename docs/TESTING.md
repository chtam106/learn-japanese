...
- Component tests render React with Testing Library; wrap UI in `<LanguageProvider initialLocale="en">` so `useTranslation()` works.
...
- Test component render React bằng Testing Library; bọc UI trong `<LanguageProvider initialLocale="en">` để `useTranslation()` hoạt động.
